...
//Function to display the Queen product on the front end

	//Create connection

	$db = new mysqli(DB_HOST, DB_USER, DB_PASS, T4_DB_NAME);

	// Query the DB for queen
	$result = $db->query("SELECT * FROM products where NAME= '$name'");
	$items = '';
	
	if(!$result){
		echo "Breaking right after query.";
		
	}
	else{
		while ($row = $result->fetch_object()) {
			//make colors colum into an array
	...
	while($i < sizeof($colors)){
                   	# for each color in row colors print out as an <option>
                   	$color_options.='<option value="'.$colors[$i].'">'.$colors[$i].'</options>';
                  $i++;
                   }
       
			$items .= '
			<div class="product">
				<h1>'.stripslashes($row->name).'</h1>
				<div class="info">
					<form action="." id="options" method="post">
    <table>
        <tr>
            <td>
                <input type="hidden" name="on0" value="Available Upgrades">Available Upgrades</td>
        </tr>
        <tr>
            <td>
                <select name="tiers" name="" onchange="document.location = this.value" value="GO">
                	 <option value="">Please Select</option>
                    <option value="index.php?action='.$_SESSION['page'].'_standard">Standard Colors</option>
                    <option value="index.php?action='.$_SESSION['page'].'_premium">Premium Colors</option>
                    <option value="index.php?action='.$_SESSION['page'].'_premier">Premier Colors</option>
                </select>
            </td>
        </tr>
        <tr>
            <td>
                <input type="hidden" name="on1" value="Color Options">Color Options</td>
        </tr>
        <tr>
            <td>
                <select id="colors" name="os1">
                    '.$color_options.'
                </select>
            </td>
        </tr>
        <tr>
            <td>
                <input type="hidden" name="on2" value="Finishes (no charge)">Finishes (no charge)</td>
        </tr>
        <tr>
            <td>
                <select name="os2">
